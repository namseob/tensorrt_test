...
def get_simple_graph_def():
	...
	with g.as_default():
		...
		e = cop.constant([[[[1.,0.5,4.,6.,0.5,1.], [1.,0.5,1.,1.,0.5,1.]]]], name='weights', dtype=dtypes.float32)
		conv = nn.conv2d(input=a, filter=e, strides=[1,2,2,1], padding='SAME', name='conv')
...
def run_graph(gdef, dumm_inp):
	...
	g = ops.Graph()
	with g.as_default():
		inp, out = importer.import_graph_def(graph_def=gdef, return_elements=['input', 'output'])
		inp = inp.outputs[0]
		out = out.outputs[0]
...
int8_infer_gdef = trt.calib_graph_to_infer_graph(int8_calib_gdef)
o2 = run_graph(int8_infer_gdef, dummy_input)
