...
class IntegrationTest(test_util.TensorFlowTestCase):
	...
	def get_simple_graph_def(self):
		...
		with g.as_default():
			...
			e = cop.constant(
							[[[[1.,0.5,4.,6.,0.5,1.],[1.,0.5,1.,1.,0.5,1.]]]], name="weights", dtype=dtypes.float32
			)
	...
	def run_graph(self, gdef, dumm_inp):
		...
		with g.as_default():
			...
			inp = inp.outputs[0]
			out = out.outputs[0]
		with self.test_session(graph=g, config=self._config, use_gpu=True, force_gpu=True) as sess:
			for _ in range(30):
